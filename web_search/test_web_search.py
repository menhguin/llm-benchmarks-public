...
async def get_completion_openai(client: OpenAI, model: str, system_prompt: str, test_question: str, context: str) -> Any:
    messages = [
...
async def get_completion_anthropic(client: anthropic.Anthropic, model: str, system_prompt: str, test_question: str, context: str) -> Any:
    ...
    if api_source == "OpenAI":
        ...
        if hasattr(response, 'content') and isinstance(response.content, list):
            for block in response.content:
                if isinstance(block, anthropic.types.ToolUseBlock) and block.name == 'web_search':
                    web_search_performed = True
                    break
...
async def process_test_case(openai_client: OpenAI, anthropic_client: anthropic.Anthropic, 
                            ...
                            test_case: TestCase) -> Tuple[bool, bool, str, float, str, str]:
    try:
        if api_source == "OpenAI":
            ...
            message_text = ""
            if hasattr(response, 'content') and isinstance(response.content, list):
                message_text = "".join(block.text for block in response.content if isinstance(block, anthropic.types.TextBlock))
        else:
...
async def web_search_unit_test(openai_client: OpenAI, anthropic_client: anthropic.Anthropic, 
                               test_model: str, test_system_prompt: str, api_source: str, comments: str) -> Tuple[DetailedTestResult, List[Tuple[bool, bool, str, float, str, str]]]:
